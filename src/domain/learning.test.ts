...
describe('explicit retrospective learning', () => {
  it('returns a proposal without mutating the source Tingo answers', () => {
    const before = [{ questionId: 'morning', optionId: 'map' }];
    ...
    expect(before).toEqual([{ questionId: 'morning', optionId: 'map' }]);
    expect(proposal.status).toBe('proposed');
  });

  it('changes the Tingo answer source only after explicit confirmation', () => {
    const answers = [{ questionId: 'morning', optionId: 'map' }];
    ...
    expect(answers).toEqual([{ questionId: 'morning', optionId: 'map' }]);
  });
});
