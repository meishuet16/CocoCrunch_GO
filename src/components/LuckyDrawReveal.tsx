...
export function LuckyDrawReveal({ result, onDraw }: LuckyDrawRevealProps) {
  ...
  return <section className="lucky-draw-reveal random-ritual" data-stage={stage} aria-labelledby="lucky-draw-title">
    <header className="lucky-draw-reveal__header"><span>LUCKY DRAW</span><h3 id="lucky-draw-title">Draw today’s little luck.</h3></header>
    ...
    <div className="fortune-scene" aria-hidden="true">
      ...
      <div className="fortune-paper"><i/><b>✦</b><i/></div>
    </div>
    ...
    <div className="random-ritual__status" role="status" aria-live="polite">
      ...
      {revealed && (result ? <div className="lucky-draw-reveal__result"><span>RESULT</span><b>{result}</b></div> : 'Waiting for the selected result…')}
    </div>
  </section>;
}
