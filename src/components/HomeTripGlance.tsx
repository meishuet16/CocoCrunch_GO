...
export function HomeTripGlance({ destination, dates, phase }: { destination: string; dates: Dates; phase: TripPhase }) {
  return <section className="home-trip-glance paper-sheet" aria-label="Trip context">
    <div className="home-trip-context"><span>TRIP CONTEXT</span><b>{destination}</b><small>{dates ? `${dates.start}–${dates.end}` : 'Dates to be confirmed'}</small></div>
    <div className="home-trip-phase"><span>CURRENT PHASE</span><b>{phaseCopy[phase]}</b></div>
