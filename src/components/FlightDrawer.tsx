...
const quotes = [
  ...
  { provider: 'CloudWays', price: 'RM 742', duration: '6h 55m', flightNumber: 'SQ 218' },
...
export function FlightDrawer({ mode, booking, draft, onDraftChange, onConfirm, onClose }: FlightDrawerProps) {
  ...
  if (booking) return <section className="flight-drawer"><span className="drawer-kicker">FLIGHT · BOOKED</span><h3>{booking.flightNumber}</h3><small className="adapter-note">Saved flight details</small><div className="flight-detail-grid"><span>Departure<b>{booking.departureTime}</b></span><span>Arrival<b>{booking.arrivalTime}</b></span><span>Check-in<b>{booking.checkInTime}</b></span><span>Source<b>{booking.provider ?? 'Email import'}</b></span></div>{mode === 'group' && <label className="flight-share"><input type="checkbox" checked={booking.sharedWithGroup} onChange={event => onConfirm({ ...booking, sharedWithGroup: event.target.checked })} /> <span>Booked for everyone</span></label>}<button className="primary" onClick={onClose}>Done</button></section>;
  return <section className="flight-drawer"><span className="drawer-kicker">ADD FLIGHT</span><h3>How would you like to add it?</h3><details open><summary>Already booked</summary><label className="setup-field"><span>Paste or forward confirmation email text</span><textarea value={draft} onChange={event => onDraftChange(event.target.value)} placeholder="MH 772 · 12 Oct · 09:15 · 16:40 · check-in 07:15" /></label><small className="adapter-note">We’ll extract the flight number and time details. Missing fields use an example value.</small><button className="secondary" onClick={confirmEmail}>Read confirmation</button></details><details><summary>Not booked yet</summary><small className="adapter-note">Example fares and voucher codes</small>{quotes.map(quote => <article className="flight-quote" key={quote.provider}><div><b>{quote.provider}</b><small>{quote.duration}{quote.voucherCode ? ` · Voucher ${quote.voucherCode}` : ''}</small></div><strong>{quote.price}</strong><button className="secondary" onClick={() => onConfirm({ flightNumber: quote.flightNumber, departureTime: '12 Oct · 09:15', arrivalTime: '12 Oct · 16:40', checkInTime: '12 Oct · 07:15', provider: quote.provider, voucherCode: quote.voucherCode, sharedWithGroup: false, source: 'quote-prototype' })}>Choose fare</button></article>)}</details><button className="onboarding-skip" onClick={onClose}>Skip for now</button></section>;
}
