...
type Props = { onAsk: () => void; onPray: () => void; onNext: () => void; onEveryday: () => void; onLucky: () => void };
const STORAGE_KEY = 'cococrunch:coco-position:v1';

export function GlobalCocoCompanion({ onAsk, onPray, onNext, onEveryday, onLucky }: Props) {
  ...
  return <aside className={`global-coco global-coco--${position.edge} ${open ? 'is-open' : ''}`} style={{ top: `${position.y * 100}%` }} aria-label="Coco companion">
    <button className="global-coco__button" type="button" aria-expanded={open} aria-label="Open Coco menu" onPointerDown={event => { drag.current = { pointerId: event.pointerId, moved: false }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={event => { if (!drag.current || drag.current.pointerId !== event.pointerId) return; if (Math.abs(event.movementX) + Math.abs(event.movementY) > 4) drag.current.moved = true; if (drag.current.moved) setPosition(current => ({ edge: event.clientX < window.innerWidth / 2 ? 'left' : 'right', y: Math.max(.08, Math.min(.92, event.clientY / window.innerHeight)) })); }} onPointerUp={event => { if (drag.current?.moved) { drag.current = null; return; } drag.current = null; setOpen(value => !value); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setOpen(value => !value); } }}><CocoCompanion context="home" pose={open ? 'expression-happy' : undefined} size={112}/></button>
