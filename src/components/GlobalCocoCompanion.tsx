...
export function GlobalCocoCompanion({ context, onAsk, onPray, onNext, onEveryday, onLucky }: Props) {
  ...
  return <aside className={`global-coco global-coco--${position.edge} ${open ? 'is-open' : ''}`} style={{ top: `${position.y * 100}%` }} aria-label="Coco companion">
    ...
    {open && <nav className={`global-coco__menu global-coco__menu--${position.edge}`} aria-label="Coco actions">
      <button className="global-coco__ask" onClick={() => activate(onAsk)}><span>Ask Coco</span><small>Type a question</small></button>
      <div className="global-coco__quick-actions">
        <button onClick={() => activate(onPray)}>Pray</button>
        <button onClick={() => activate(onEveryday)}>Everyday Gacha</button>
      </div>
    </nav>}
  </aside>;
}
