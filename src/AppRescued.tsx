...
import { applyActualAdjustment, normalizeBudgetActuals, paceEvidenceSummary, rateDecision, updateBudgetActual } from './domain/retrospective';
import { loadPersisted, savePersisted, type CompletedPaceEvidence, type CourtOptionState, type DecisionRecord } from './persistence';
...
export default function AppRescued() {
  ...
  }, [mode, destination, profile, plannerTurn, courtVotes, courtConfirmed, courtDecision, courtOptions, activeConflict, decisionHistory, groupBudgetTotal, soloBudgetTotal, groupBudgetPlan, soloBudgetPlan, groupBudgetActuals, soloBudgetActuals, delay, mood, arrivalChecked, privacy, continuousLocation, recommendations, worthIt, profileLearned, tingoAnswers, tingoDimensions, basePackingPreferences, tripCreated, members, constraints, reminders, commitments, reunion, published, memoryNote, memoryPublic, itemReviews]);
  ...
  function applyRepair() {
    if (!replanApplied) {
      if (mode === 'group') setGroupBudgetActuals(actuals => applyActualAdjustment(actuals, 'activities', 8));
      else setSoloBudgetActuals(actuals => applyActualAdjustment(actuals, 'activities', 8));
    }
  ...
  function undoRepair() {
    if (replanApplied) {
      if (mode === 'group') setGroupBudgetActuals(actuals => ({ ...actuals, activities: Math.max(0, actuals.activities - 8) }));
      else setSoloBudgetActuals(actuals => ({ ...actuals, activities: Math.max(0, actuals.activities - 8) }));
    }
  ...
  function openGovernedAction(action: Exclude<GovernedAction, null>) {
    ...
    const proposal = action === 'assistant-move'
      ? { topic: 'Coco café timing', options: [{ id: 'keep-cafe', label: `Keep café at ${cafeTime}` }, { id: 'move-cafe', label: 'Move café to 15:00' }] }
      : action === 'split-on'
  ...
  function renderDuring() {
    return <>
      ...
      {replanApplied && <section className="success-note"><Check size={21}/><div><b>Plan repaired.</b><small>Anchor protected · Mystery Window moved · +RM8 recorded in Activities actuals · undo available</small></div><button onClick={undoRepair}>Undo</button></section>}
      ...
      {mode === 'group' && <section className="split-note"><div><span>SMART SPLIT</span><b>{split ? '2 café · 2 shopping' : 'Different energy levels?'}</b><small>{split ? 'Reunion · 19:30 · ±15 min · official shared state' : 'Preview is harmless; creating the official split requires Group Court.'}</small>{split && <div className="split-timelines"><span>Mei + Zi Shan · café · 14:30</span><span>JH + Alex · shopping · 14:30</span></div>}</div><button onClick={() => openGovernedAction(split ? 'split-off' : 'split-on')}>{split ? 'Request reunion' : 'Propose split'}</button></section>}
