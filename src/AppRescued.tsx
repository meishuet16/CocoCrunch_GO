...
export default function AppRescued() {
  ...
  useEffect(() => {
    savePersisted({
      ...
      members, memberPreferenceProfiles, backupCandidates, appliedRepair: appliedRepair ?? undefined, constraints, reminders, commitments, reunion, published, memoryPublic, itemReviews, revivedWishIds: ghostWishes.filter(wish => wish.status === 'revived').map(wish => wish.id), groupName, destinationLockedByLeader, datesLockedByLeader, groupMemberBudgets, groupMemberVibes, groupMemberDestinations, itineraryOrder, groupChannelMessages, groupCourtUnreadCount, groupSplitPlan, photoMemoryArtifacts, flightBooking, flightBookingDraft, accommodationBooking, accommodationBookingDraft,
    });
  }, [mode, destination, readyConfirmed, profile, plannerTurn, courtVotes, courtConfirmed, courtDecision, courtOptions, activeConflict, decisionHistory, groupBudgetTotal, soloBudgetTotal, groupBudgetPlan, soloBudgetPlan, groupBudgetActuals, soloBudgetActuals, delay, mood, arrivalChecked, privacy, continuousLocation, recommendations, tripIntent, worthIt, profileLearned, learningProposal, confirmedLearningHistory, tingoAnswers, tingoDimensions, onboardingComplete, onboardingName, onboardingCountryCode, onboardingBirthday, basePackingPreferences, tripCreated, tripPhase, members, memberPreferenceProfiles, backupCandidates, appliedRepair, constraints, reminders, commitments, reunion, published, memoryPublic, itemReviews, ghostWishes, groupName, destinationLockedByLeader, datesLockedByLeader, groupMemberBudgets, groupMemberVibes, groupMemberDestinations, itineraryOrder, groupChannelMessages, groupCourtUnreadCount, groupSplitPlan, photoMemoryArtifacts, automaticDeviationPrompted, completedTodayItemIds, tripEndPromptDismissed, emergencyContacts, emergencyCheckInFrequency, selectedEmergencyContactId, flightBooking, flightBookingDraft, accommodationBooking, accommodationBookingDraft]);
  ...
  function renderTrips() {
    if (!tripSetupModeChoice && tripCreated) return <section className="trips-index" aria-label="Trips"><span className="trips-index-heading">{mode === 'group' ? 'GROUP TRIPS' : 'SOLO TRIPS'}</span><button className="trips-index-card cc-card" onClick={openTrip}><CocoCompanion context="planning" pose="action-map" size={58} /><div><b>{groupName || destination}</b><small>{destination || 'To be confirmed'}</small></div><ChevronRight size={18} /></button><button className="primary" onClick={() => setTripSetupModeChoice(true)}>Add Trip <ChevronRight size={16} /></button></section>;
