...
type TripListEntry = { id: string; name: string; destination: string; mode: TripMode; status: 'planning' | 'ongoing' | 'completed'; createdAt: string };
...
export default function AppRescued() {
  ...
  function renderTrips() {
    if (!tripSetupModeChoice && tripList.length) return <section className="trips-index" aria-label="Trips">{(['group', 'solo'] as const).map(kind => { const trips = tripList.filter(trip => trip.mode === kind && trip.status === 'planning'); return trips.length ? <section className="trip-list-section" key={kind}><span className="trips-index-heading">{kind === 'group' ? 'GROUP TRIPS' : 'SOLO TRIPS'}</span>{trips.map(trip => <button className="trips-index-card cc-card" key={trip.id} onClick={() => { setCurrentTripListId(trip.id); openTrip(); }}><CocoCompanion context="planning" pose={kind === 'group' ? 'action-map' : 'action-journal'} size={58} /><div><b>{trip.name}</b><small>{trip.destination || 'To be confirmed'}</small></div><ChevronRight size={18} /></button>)}</section> : null; })}<button className="primary" onClick={() => setTripSetupModeChoice(true)}>Add Trip <ChevronRight size={16} /></button></section>;
