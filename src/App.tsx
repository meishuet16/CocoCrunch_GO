...
export default function App() {
  ...
  function renderPlan() {
    return <>
      ...
      {mode === 'group' && <section className="conflict-ticket"><span>UNRESOLVED CONFLICT</span><b>Ramen tonight vs sushi tonight</b><small>2–2 tie · Gacha unlocks only after a real tie</small><button className="ritual-trigger" onClick={() => { setCourtOpen(true); setGacha(null); setCourtConfirmed(false); }}>Open Group Court <Gavel size={18}/></button></section>}
    </>;
  }
  ...
  function renderMemories() {
    return <>
      ...
      <section className="trunk-hero"><div className="trunk-lid"/><div className="trunk-body"><span className="postcard p1">TOKYO</span><span className="postcard p2">雨の日</span><span className="ticket">10.13</span><Coco tiny mood="happy"/></div></section>
      <section className="memory-actions"><button onClick={() => setPhotoIndexed(true)}><Map size={20}/><span><b>Photo Map</b><small>{photoIndexed ? '18 photos indexed · 4 areas' : 'Import EXIF placeholder'}</small></span></button><button onClick={() => setJournalGenerated(true)}><BookOpen size={20}/><span><b>Travel journal</b><small>{journalGenerated ? 'Draft generated' : 'Generate from timeline + photos'}</small></span></button></section>
  ...
  function renderMe() {
    return <>
      ...
      <section className="profile-fields"><label><span>Trip vibe</span><input value={tripVibe} onChange={e => setTripVibe(e.target.value)}/></label><label><span>Must-Go</span><input value={mustGo} onChange={e => setMustGo(e.target.value)}/></label><label><span>Deal breaker</span><input value={veto} onChange={e => setVeto(e.target.value)}/></label><label><span>Preference</span><input value={preference} onChange={e => setPreference(e.target.value)}/></label><label><span>Flexible</span><input value={flexible} onChange={e => setFlexible(e.target.value)}/></label></section>
