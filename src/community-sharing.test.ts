...
describe('buildPublicCommunityTrip', () => {
  it('passes only per-card public memories into the Explore payload', () => {
    const result = buildPublicCommunityTrip({ published: true, id: 99, title: 'Tokyo', author: 'Priya', destination: 'Tokyo', artifacts: [{ id: 'private', title: 'Private tea', body: 'secret', source: '', locationLabel: 'Tokyo', audience: 'personal', isPublic: false }, { id: 'public', title: 'Public ramen', body: 'share', source: '', locationLabel: 'Tokyo', audience: 'personal', isPublic: true }] });
    ...
    expect(buildPublicCommunityTrip({ published: false, id: 99, title: 'Tokyo', author: 'Priya', destination: 'Tokyo', artifacts: [] })).toBeNull();
  });
});
